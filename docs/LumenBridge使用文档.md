...
聊天美化 / 格式化类插件（如 u_beautiful_chat）会取消原版聊天事件，再用自己的格式重新广播消息。v1.0.6 起 LumenBridge 能自动识别这种「取消后重发」的行为并照常转发玩家发言，无需额外配置。两点说明：转发到群的是玩家输入的原始发言（不带游戏内格式的维度、生命值等修饰，这些只对游戏内显示有意义）；如果需要群侧也过滤敏感词，请使用 LumenBridge 自带的聊天屏蔽功能，美化插件自己的屏蔽词表不会作用于群消息。若某插件取消聊天后是「逐玩家私发」而非广播（此模式极少见），LumenBridge 会视为消息未展示而不转发。
