...
自定义页面在 iframe 中加载，主面板会自动注入基础融合样式（`html,body { background: transparent !important; }`）并按内容高度自适应 iframe：

- 画布透明：页面直接衬在主面板背景上，与普通页面观感一致，不会呈现一块自带底色的"嵌入式网页方框"；
- 移动端全宽：iframe 横向铺满整个视口，与底部导航栏同宽；
- 高度自适应（两者自动生效，无需关心）：
  - **自然流页面**（不定高）：iframe 高度随内容延伸，内容随整页滚动到底部导航栏之下（导航栏悬浮其上）；
  - **满屏型页面**（`html,body{height:100%}`）：iframe 至少被撑到一屏（视口底部），页面内的自适应布局照常工作；内容更多时建议内部滚动或改为自然流。

兼容性：页面**无需自带**任何高度上报脚本——主面板会在加载后自动注入。若历史版本页面已内嵌旧版上报脚本也不影响（新版使用独立的幂等标志，旧脚本的上报会被主面板忽略）。

如需保留自己的页面背景色，用内联样式覆盖（内联 `!important` 优先级最高）：
